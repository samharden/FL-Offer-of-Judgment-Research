...
| `practice-guide.md` | ⭐ **The practitioner deliverable (built Aug. 11, 2026)** — the regime as a working tool rather than a history. Twelve parts: what the device does · the serve decision · the calendar · joint proposals · drafting checklist · service · receiving a proposal · the 25% comparison · claiming fees · when § 768.79 does not apply · thirteen ranked traps · unsettled ground. Plus a one-page drafting checklist and a table of authorities. **Every case name hyperlinked — 191 mentions, 48 cases.** Citations re-verified Aug. 11 |
| `visuals/practice-guide.html` | The practice guide as a standalone page — sidebar TOC, light/dark, print stylesheet. **Generated from `practice-guide.md`; do not hand-edit.** Rebuild with the script in the session scratchpad, or regenerate from the Markdown |
...
| **4** — synthesis | **Substantially complete (Aug. 4; practice guide added Aug. 11).** Arc matrix, alignment matrix, and two-track timeline in `arc-matrix.md`; the survey memo in `memo.md`; the timeline visual in `visuals/`; the case database in `case-database.csv`; ✅ **the practitioner deliverable in `practice-guide.md` (+ HTML build)**. **Remaining: the district-split matrix (needs the DCA corpus), and a second pass once rehearing resolves** |
...
5. ✅ ~~Practitioner checklist~~ — **done August 11, 2026, and scoped up.** Delivered as a full **practice guide** (`practice-guide.md` + `visuals/practice-guide.html`), not a bare checklist; the checklist survives as its one-page appendix. Built on the *Anderson*/*Audiffred* "speaks for or binds" line as planned, plus the *Gorka* / all-or-nothing structural distinction and the **conjunctive 25% test** from *Hoang Dinh Duong*.
   - ⚠️ **It inherits the *Trace Elements* exposure.** Part 4 is the part that moves if clarification is granted. The guide carries the August 8 date in a red banner and in its footer — **update both when the docket is re-pulled.**
   - ⚠️ **Three district cites were dropped, not verified.** *Ganz*, *Tampa Letter Carriers*, and *Cooper v. Marriott* — cited in *Advanced Chiropractic* for the *Stockman* pleading exemption — have no reporter cites recorded in this project, so they were removed rather than link-guessed. Restore them if the proposition is ever load-bearing.
